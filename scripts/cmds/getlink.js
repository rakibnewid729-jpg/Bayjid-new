...
const baseApiUrl = "http://noobs-api.top/dipto"
    ...
    try {
      ...
      if (
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            let imgLink = await get(
              `${baseApiUrl}/imgbb?url=${encodeURIComponent(messageReply.attachments[i].url)}`,
            );
            num += 1;
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            let shortLink = await shortenURL(messageReply.attachments[i].url);
            const res = await get(`${baseApiUrl}/tg?url=${shortLink}`);
            num += 1;
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            let shortLink = await shortenURL(messageReply.attachments[i].url);
            const res = await get(
              `${baseApiUrl}/imgur?url=${shortLink}`,
            );
            num += 1;
            msg += `${num}: ${res.data.data}\n`;
          }
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            const { data } = await get(
              `${baseApiUrl}/linkshort?link=${encodeURIComponent(messageReply.attachments[i].url)}name=${encodeURIComponent(messageReply.attachments[i].filename)}`,
            );
            num += 1;
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            const encLink = encodeURIComponent(messageReply.attachments[i].url);
            const res = await get(
              `${baseApiUrl}/postimg?url=${encLink}`,
            );
            num += 1;
        ...
        if (!messageReply.attachments || length == 0) {
          ...
          for (let i = 0; i < length; i++) {
            const encLink = encodeURIComponent(messageReply.attachments[i].url);
            const res = await get(
              `${baseApiUrl}/drive?url=${encLink}`,
            );
            num += 1;
            msg += `${num}: ${res.data.fileUrl}\n`;
          }
          message.reply(msg);
        }
      }
